...
//				+ "| here. "
	...
	private String[][] getOutput() {
		...
		return new String[][]{
			...
			{"Rectangle accepted:(r3, 5, 6, 7, 8)"},
			{"Rectangle accepted:(rand46, 561, 434, 277, 401)"},
			{"Rectangle accepted:(rand47, 250, 200, 100, 50)"},
			...
			{"Rectangle accepted:(r4, 0, 0, 3, 5)"},
			{"Rectangle accepted:(rect9, 73, 4, 12, 45)"},
			{"Rectangle accepted:(xyz, 241, 320, 494, 124)"},
			{"Rectangle found: (r3, 200, 120, 150, 100)", 
				"Rectangle found: (r3, 5, 6, 7, 8)",
				"Rectangle found: (r3, 483, 274, 63, 19)",
				"Rectangle found: (r3, 1, 5, 21, 3)"},
			{"Rectangle found: (r4, 0, 0, 3, 5)"}, {"Rectangle not found: r5"}
		};
	}

	private String[] getParams() {
		// TODO Auto-generated method stub
		return new String[]{"SearchTest.txt"};
	}
...
//			String pattern = "(" + nodeInfo[i].replaceAll("\\s", "|") + ")(\\D|$)";
//			Pattern r = Pattern.compile(pattern);
//			Matcher m = r.matcher(actualOutputLine);
//			int count =0;
//			while(m.find())
//				count++;
//			if(count>=4){ 	// all information found
//				if(visited[i])
//					continue;
//				numVisited++;
//				visited[i]=true;
...
//						if(visited[j])
//							throw new Exception("You forgot to report the rectangle"
